...
static void build_rank_p(k2bp_traversal_t* pos_a, k2bp_t* a);
static void get_node(k2bp_traversal_t* pos_a, const k2bp_t* a, size_t *node, size_t *size, size_t *leaves);
static void build_leaves_pointers(k2bp_traversal_t* pos_a, k2bp_t* a);
...
static uint64_t k2bp_leaves_between_pointers(const k2bp_t* a, size_t rank_r, size_t rank_l);
...
static uint64_t rank_p(const k2bp_traversal_t* pos_a, const k2bp_t* a) {
  ...
  uint64_t ret = iv_get(&(a->rank_pointers), block);
...
static void build_leaves_pointers(k2bp_traversal_t* pos_a, k2bp_t* a) {
  ...
  assert(a->rank_pointers.data != NULL);
...
//  iv_init(&(pos_a->size_sub_pointers), a->n_p, ceil_log2(a->t.n / 2));
  iv_init(&(a->leaves_pointers), a->n_p, ceil_log2(a->n_l));
  iv_init(&(a->node_pointers), a->n_p, ceil_log2(a->n_info));
  iv_init(&(a->size_sub_pointers), a->n_p, ceil_log2(a->t.n / 2));
...
//    iv_set(&(pos_a->size_sub_pointers), i, pos_start.size);
    iv_set(&(a->node_pointers), i, p_node);
    iv_set(&(a->size_sub_pointers), i, pos_start.size);
  }
}

static void build_rank_p(k2bp_traversal_t* pos_a, k2bp_t* a) {
  //if(pos_a->rank_pointers.data != NULL) return;
  if(a->rank_pointers.data != NULL) return;
  ...
  iv_init(&(a->rank_pointers), SAMPLE_SIZE_RANK(a->t.n), ceil_log2(a->n_p));
  size_t block = 0;
  ...
  for(i = 0; i + 64 < a->t.n; i += 64) {
    if(i % BLOCK_SIZE_RANK == 0) {
      ...
      iv_set(&(a->rank_pointers), block, prefix_sum);
      block++;
    }
...
static void precompute_info(k2bp_traversal_t* pos_a, k2bp_t* a) {
  ...
  if(HAS_POINTERS(a)) {
    ...
    pos_a->leaves -= iv_get(&(a->leaves_pointers), a->n_p - 1);
    pos_a->flag_p = 0;
  }
}
...
//    iv_free(&(pos_a->size_sub_pointers));
    iv_free(&(a->rank_pointers));
    iv_free(&(a->leaves_pointers));
    iv_free(&(a->node_pointers));
    iv_free(&(a->size_sub_pointers));
  }
}
...
//                           [k2bp_read_leaf(b, pos_b->i_l + (r_b > 0 ? iv_get(&(pos_b->leaves_pointers), r_b - 1) : 0))];
    uint8_t res = table_mul[k2bp_read_leaf(a, pos_a->i_l + (r_a > 0 ? iv_get(&(a->leaves_pointers), r_a - 1) : 0))]
                           [k2bp_read_leaf(b, pos_b->i_l + (r_b > 0 ? iv_get(&(b->leaves_pointers), r_b - 1) : 0))];
    if(res == 0) {
      bv_pb(&(c->t), 1);
      bv_pb(&(c->t), 0);
    } else {
...
//                       k2bp_read_leaf(b, pos_b->i_l + (r_b > 0 ? iv_get(&(pos_b->leaves_pointers), r_b - 1) : 0)));
    k2bp_write_leaf(c, k2bp_read_leaf(a, pos_a->i_l + (r_a > 0 ? iv_get(&(a->leaves_pointers), r_a - 1) : 0)) |
                       k2bp_read_leaf(b, pos_b->i_l + (r_b > 0 ? iv_get(&(b->leaves_pointers), r_b - 1) : 0)));
    pos_a->i_t += 4;
    pos_b->i_t += 4;
    pos_b->i_l += 1;
...
static uint64_t k2bp_leaves_between_pointers(const k2bp_t* a, size_t rank_r, size_t rank_l) {
  ...
  if(rank_l == 0) {
    ...
    return iv_get(&(a->leaves_pointers), rank_r - 1);
  }
  ...
  return iv_get(&(a->leaves_pointers), rank_r - 1) - iv_get(&(a->leaves_pointers), rank_l - 1);
}
...
static uint8_t k2bp_check_and_move(k2bp_traversal_t* pos_a, const k2bp_t* a, k2bp_traversal_t* new_pos_a) {
  if(bv_get_int(&(a->t), pos_a->i_t, 6) == LEAF_P) {
    ...
    new_pos_a->i_l = pos_a->i_l + k2bp_leaves_between_pointers(a, pos_a->i_p, new_pos_a->i_p);
...
//    new_pos_a->size = iv_get(&(pos_a->size_sub_pointers), pos_a->i_p);
    new_pos_a->node = iv_get(&(a->node_pointers), pos_a->i_p);
    new_pos_a->size = iv_get(&(a->size_sub_pointers), pos_a->i_p);
