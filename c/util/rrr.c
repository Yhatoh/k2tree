#include <assert.h>
#include <stdint.h>
#include <stddef.h>
#include <stdlib.h>
#include "rrr.h"
#include "../k2bp/util.h"
#include "rrr_util.h"

void rrr_compress(rrr_t* rrr, uint8_t b, uint8_t* bits, size_t n) {
  rrr->n = n;
  rrr->b = b;
  rrr->logb = ceil_log2(b + 1);
  size_t pairs = (n + rrr->b - 1) / rrr->b;
  size_t pairs_per_64 = 64 / rrr->logb;
  iv_init(&(rrr->c), pairs, rrr->logb);
  bv_init(&(rrr->o));
  size_t curr_bit_o = 0;
  size_t curr_pair = 0;
  for(size_t i = 0; i < n; i += rrr->b) {
    size_t class = 0;
    for(size_t j = i; j < i + rrr->b && j < n; j++) {
      if(bits[j / 8] & (1 << (j % 8))) 
        class++;
    }

    size_t class_ = class;
    uint64_t offset = 0;
    size_t j = 0;
    while(class_ > 0 && class_ <= rrr->b - j - 1) {
      if(bits[(j + i) / 8] & (1 << ((j + i) % 8))) {
        offset += comb[rrr->b - j - 1][class_];
        class_--;
      }
      j++;
    }
    iv_set(&(rrr->c), curr_pair, class);
    bv_append_uw(&(rrr->o), offset, l[class] + 1);
    curr_pair++;
  }
  bv_shrink(&(rrr->o));
}

void rrr_free(rrr_t* rrr) {
  rrr->n = rrr->b = 0;
  iv_free(&(rrr->c));
  bv_free(&(rrr->o));
}

void rrr_decompress(rrr_t* rrr, uint8_t** bits, size_t* n) {
  *n = rrr->n;
  *bits = (uint8_t*) calloc((rrr->n + 8 - 1) / 8, sizeof(uint8_t));

  size_t curr_bit_o = 0;
  size_t curr_bit = 0;
  for(size_t i = 0; i < rrr->c.n; i++) {
    size_t class = iv_get(&(rrr->c), i);
    uint64_t offset = bv_get_int(&(rrr->o), curr_bit_o, l[class] + 1);
    curr_bit_o += l[class] + 1;

    size_t j = 0;
    while(class > 0) {
      if(offset >= comb[rrr->b - j - 1][class]) {
        (*bits)[(curr_bit + j) / 8] |= (1 << ((curr_bit + j) % 8));
        offset -= comb[rrr->b - j - 1][class];
        class--;
      }
      j++;
    }
    curr_bit += rrr->b;
  }
}
